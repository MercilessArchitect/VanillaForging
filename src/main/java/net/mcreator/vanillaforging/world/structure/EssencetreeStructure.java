...
public class EssencetreeStructure extends VanillaforgingModElements.ModElement {
	public EssencetreeStructure(VanillaforgingModElements instance) {
		super(instance, 206);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 20000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("vanillaforging", "essence_tree"));
						if (template == null)
							return false;
