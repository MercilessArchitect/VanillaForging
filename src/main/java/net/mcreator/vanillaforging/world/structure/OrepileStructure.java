...
public class OrepileStructure extends VanillaforgingModElements.ModElement {
	public OrepileStructure(VanillaforgingModElements instance) {
		super(instance, 205);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 50000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("vanillaforging", "ore_pillar"));
						if (template == null)
							return false;
