...
package net.mcreator.vanillaforging.world.biome;
...
public class OrousforestBiome extends VanillaForgingElements.ModElement {
	...
	public void init(FMLCommonSetupEvent event) {
		...
		BiomeManager.addBiome(BiomeManager.BiomeType.WARM, new BiomeManager.BiomeEntry(biome, 128));
	}
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.SAVANNA).waterColor(-16777114).waterFogColor(-16777114).parent("savanna")
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.PODZOL.getDefaultState().getBlock() || ground == Blocks.COARSE_DIRT.getDefaultState().getBlock())
							&& (ground2 == Blocks.PODZOL.getDefaultState().getBlock() || ground2 == Blocks.COARSE_DIRT.getDefaultState().getBlock())))
						return false;
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == IronwoodleavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, IronwoodleavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, IronwoodLogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == IronwoodLogBlock.block.getDefaultState().getBlock()
					|| blockType == IronwoodleavesBlock.block.getDefaultState().getBlock() || blockType == Blocks.PODZOL.getDefaultState().getBlock()
