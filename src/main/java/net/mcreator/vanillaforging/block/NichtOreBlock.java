...
import net.mcreator.vanillaforging.item.NichtshardItem;
...
public class NichtOreBlock extends VanillaforgingModElements.ModElement {
	@ObjectHolder("vanillaforging:nichtore")
	public static final Block block = null;
	public NichtOreBlock(VanillaforgingModElements instance) {
		super(instance, 4);
	}
	...
	public static class CustomBlock extends Block {
		...
		public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
			...
			return Collections.singletonList(new ItemStack(NichtshardItem.block, (int) (3)));
		}
	}
	@Override
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("nichtore", "nichtore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 6)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(8, 16, 16, 50))));
		}
	}
}
